...
- **1.1 (incl. 1.1.1 / 1.1.3)** -- bundled Windows installer (`install-healthcheck.ps1`, now parses on PowerShell 5.1 and 7, sets up TLS for corporate proxies) and the `--check-connection` connectivity pre-flight; built-in scan templates are labelled rather than suppressed and now visually badged in the report (corrected/expanded id set, 22 templates); two credential-governance audit rules; the agent-only coverage check is now exact instead of sampled; "sites with zero assets" is one finding per site; hierarchical terminal progress (and INFO no longer clutters the terminal in normal runs) and a leaner release ZIP.
...
- **`install-healthcheck.ps1` errors with `missing )` / `missing }` (or won't start) on Windows PowerShell**: you're running a pre-1.1.1 copy of the installer under Windows PowerShell 5.1, which mis-reads the script's encoding. Use the installer from the **1.1.1 or newer** release zip -- it parses cleanly on both PowerShell 5.1 and 7.
- **`SSLError: CERTIFICATE_VERIFY_FAILED` / `unable to get local issuer certificate`** (Windows, especially on a corporate network): the host's cert is signed by a CA your Python install doesn't trust -- typically because a corporate proxy (Zscaler, Palo Alto, Netskope, etc.) re-signs TLS traffic with an internal CA that Windows trusts but `requests` does not. The 1.1.1+ `install-healthcheck.ps1` installs `pip-system-certs` automatically, which fixes this for most setups. If you installed manually or still hit it, run `pip install pip-system-certs` in the same venv and re-run -- that makes `requests` use the Windows trust store. If your IT team can supply the proxy's root CA as a `.pem`, you can alternatively set `REQUESTS_CA_BUNDLE=path\to\ca.pem`. Setting `verify_tls: false` in `config.yaml` disables verification entirely -- last resort, never for production.
