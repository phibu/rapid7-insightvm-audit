...
The set of assets that belong to the **agent site** but to **no scan site** — i.e. assets the console only ever sees through the endpoint agent, never via a network scan. This is a *site-membership* question (`agent_site_assets − union(scan_site_assets)`), **not** a scan-*scope* question (whether an asset's IP falls in some site's `included_targets`) — the two give different numbers, and the membership definition is the canonical one (see [docs/adr/0004-agent-only-coverage-gap-server-side.md]). It is computed **server-side and complete** (filtered asset search by site name, count from `page.totalResources`), never by sampling-and-extrapolating — the old sampled rule existed only to dodge a per-asset-GET loop that the server-side query removes.
