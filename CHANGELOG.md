...
- **Internal:** the report view-model builders moved out of `__main__` (which CONTEXT.md scopes to wiring only) into the report layer: `report.build_inventory_totals(snapshot)` and `report.build_topology(snapshot)` (was `__main__._build_inventory_totals` / `_build_topology`). The figure→section binding is also consolidated: both check-name constants (`_COVERAGE_CHECK_NAME`, `_SCAN_ENGINES_CHECK_NAME`) now live in `report.py`, and `diagrams.extract_coverage_counts` takes the coverage check name as a `check_name=` argument instead of reading a module global — so `diagrams.py` no longer hardcodes which check it reads and `report.py` no longer reaches into a `diagrams` private. No behavior change; `__main__` shrinks to wiring. See CONTEXT.md "Report diagram".
- **Internal:** the archify cardinal two-rect mask rule (an opaque `dg-mask` rect under each styled box of identical geometry) was open-coded at seven box sites across the three diagram builders; it now lives in one helper, `diagrams._masked_rect(parts, x, y, w, h, cls, *, rx=6)`. A theming-mask fix is now one edit, not seven. SVG output is byte-identical (the existing diagram tests verify it); a new `test_diagrams_mask_invariant` locks "every styled box is masked" so a future builder can't silently drop the mask. See CONTEXT.md "Report diagram".
...
- **Report:** the Scan Engines section now renders an inline-SVG **scan-topology diagram** — an *engine-centric* columnar layout (Sites → Engines/Pools). Sites are aggregated into a paired-sites bucket and an orphan-sites bucket rather than drawn individually, so the figure stays legible and the layout stays collision-free on consoles with hundreds of sites (the node count is bounded by engines + pools, not sites). Each engine card shows its site count, asset load, pool membership, and an overloaded flag (matching the `single_engine_overload` rule); pools wrap their members in a dashed boundary; unpaired engines (no direct sites and in no pool) get their own bucket. The topology numbers come from a `TopologyData` view-model built in `__main__` from **already-cached** snapshot reads (`sites`, `scan_engines`, `scan_engine_pools`, the inline `site_asset_count`) — **no new API calls** — and carried on `ReportContext` beside `inventory_totals`; `diagrams.py` stays API-free. Same theming/omit-when-absent discipline as the coverage figure. Covered by `test_diagrams_topology.py` (pairing/orphan/unpaired/pool/overload aggregation + SVG theming + report integration).
- **Report:** the Summary section now renders an inline-SVG **health status map** — one stacked bar per check (those with rule results), with segments proportional to its per-rule status counts in pass → warn → fail → error → skipped order plus a legend, so the report reads as a triage board (which subsystem is healthy, which has failures, how many rules drive it). `error` and `skipped` get their own non-health segment colors (info / muted) and are never folded into `pass`. Reads only the already-computed per-rule statuses (`extract_status_map`) — **no new API calls, no snapshot data**; built by the same pure `diagrams.py` pattern and themed via `dg-seg-*` classes off the report's status palette. Omitted when no check carries rule results. Covered by `test_diagrams_status.py`.
