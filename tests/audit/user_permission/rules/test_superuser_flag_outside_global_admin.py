...
def test_pass_when_superuser_only_on_global_admin(fake_snapshot):
    ...
    r = SuperuserFlagOutsideGlobalAdminRule().run(fake_snapshot, "fail", False, 500, {})
    assert r.status == "pass"


def test_fails_when_superuser_on_custom_role(fake_snapshot):
    fake_snapshot.set_users([
