...
def test_pass_when_disabled_users_have_no_bindings(fake_user_snapshot):
    fake_user_snapshot.set_users([
        {"id": 1, "login": "ghost", "enabled": False, "role": {}},  # no role.id
    ])
    r = DisabledUserWithRoleBindingsRule().run(fake_user_snapshot, "info", False, 500, {})
    assert r.status == "pass"


def test_pass_when_users_with_bindings_are_enabled(fake_user_snapshot):
    fake_user_snapshot.set_users([
...
def test_info_finding_for_disabled_user_with_role(fake_user_snapshot):
    fake_user_snapshot.set_users([
...
def test_finding_severity_inherits_rule_severity(fake_user_snapshot):
    """If operator overrides severity to warn, status escalates."""
    fake_user_snapshot.set_users([
        {"id": 1, "login": "stale", "enabled": False, "role": {"id": "user"}},
    ])
    r = DisabledUserWithRoleBindingsRule().run(fake_user_snapshot, "warn", False, 500, {})
