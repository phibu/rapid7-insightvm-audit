...
def test_pass_with_two_gas(fake_user_snapshot):
    fake_user_snapshot.set_users([_ga(1, "alice"), _ga(2, "bob"), _user(3, "carol")])
    r = MultipleGlobalAdministratorsRule().run(fake_user_snapshot, "warn", False, 500, {})
    assert r.status == "pass"


def test_warn_with_three_gas(fake_user_snapshot):
    fake_user_snapshot.set_users([_ga(1, "a"), _ga(2, "b"), _ga(3, "c")])
    r = MultipleGlobalAdministratorsRule().run(fake_user_snapshot, "warn", False, 500, {})
...
def test_disabled_gas_not_counted(fake_user_snapshot):
    fake_user_snapshot.set_users([_ga(1, "a"), _ga(2, "b"), _ga(3, "c", enabled=False)])
    r = MultipleGlobalAdministratorsRule().run(fake_user_snapshot, "warn", False, 500, {})
    assert r.status == "pass"  # only 2 enabled GAs


def test_threshold_knob(fake_user_snapshot):
    fake_user_snapshot.set_users([_ga(i, f"a{i}") for i in range(5)])
    r = MultipleGlobalAdministratorsRule().run(
        fake_user_snapshot, "warn", False, 500, {"max_global_administrators": 5},
    )
    assert r.status == "pass"
...
def test_zero_global_admins_hard_fails(fake_user_snapshot):
    ...
    r = MultipleGlobalAdministratorsRule().run(fake_user_snapshot, "info", False, 500, {})
