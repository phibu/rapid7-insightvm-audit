...
- The **health status map** (third diagram) needed no new data at all — it reads the per-rule statuses already on each `CheckResult.rule_results`. It is the one figure that spans all checks rather than belonging to one, so it renders in the Summary section as its own `status_map_svg` template variable, not via the per-check `section_diagrams` map. `error`/`skipped` carry their own non-health segment colors so the bar never overstates health.
