...
# This scanner prints back the source lines it flags, and the loudest tell it looks
# for is emoji -- so its own output routinely contains characters a legacy console
# encoding (Windows cp1252, some CI locales) cannot encode. Left alone, the first
# emoji in a report crashes the run with UnicodeEncodeError. Re-open the text streams
# as UTF-8 with errors="replace" so a stray unencodable byte degrades to a placeholder
# instead of aborting the scan. reconfigure() exists on 3.7+ TextIOWrapper; guard it
# in case stdout was swapped for something that lacks it (a pipe, a test capture).
