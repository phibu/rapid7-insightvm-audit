...
- `scripts/unslop_code_scan.py` - a standalone multi-language scanner (Python standard library
  only).
- `scripts/test_unslop_code_scan.py` - the scanner's test suite (standard library, no pytest;
  run `python3 scripts/test_unslop_code_scan.py`). Covers each surface tell, the `unslop-ignore`
  escape hatch, the class/severity tagging, and a regression test that the scanner's own
  emoji-bearing output does not crash a legacy (cp1252) console.
