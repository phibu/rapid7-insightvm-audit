...
The trap has a second face that hides as helpfulness: **scope-padding**. The same impulse that
adds a needless type also adds a `--recursive` flag, a second config parameter, an env-var
fallback, an extra output format — capability the user never asked for. It feels like
thoroughness, but it is the same average-of-careful-code default, and it reads as AI for the same
reason: it is responding to "what a complete version of this looks like in the training set"
rather than to the request in front of you. Build exactly what was asked. If a fuller version
might genuinely be wanted, name it and ask — a one-line "want me to also handle nested folders?"
beats silently shipping the flag. Surplus capability is a tell, not a courtesy.
